...
// Verify if user is turista
const verifyTurista = async (req, res, next) => {
  try {
    const token = req.headers.authorization?.split(' ')[1];
    ...
    if (decoded.rol !== 'turista' && decoded.rol !== 'admin') {
      return res.status(403).json({
        success: false,
        message: 'No tienes permiso para realizar esta acción. Debes ser turista.'
      });
    }
